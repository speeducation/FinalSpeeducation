...
from apps.lineas.models import Linea, Actividad
from .models import Alumno, PlanEstudio, Evaluacion
from django.shortcuts import render, get_object_or_404
from .forms import AgregarAlumno, AgregarPlan, AgregarEvaluacion
...
@login_required(login_url='/')
def detalles_plan(request, pk):
    url = get_base_url(request)
    pk_alumno = (get_base_url(request).split('/plan')[0].split('/alumno/'))[1]
    alumno = Alumno.objects.get(pk=pk_alumno)
    plan = get_object_or_404(PlanEstudio, pk=pk)
    linea = Linea.objects.get(pk=plan.linea.pk)
    evaluaciones = Evaluacion.objects.filter(plan = plan)
    context = {
        'plan' : plan,
        'alumno' : alumno,
    ...
    return render(request, 'alumnos/detalles_plan.html', context)
...
@login_required(login_url='/')
def agregar_evaluacion(request, pk):
    url_old = get_base_url(request).split('/linea')[0]
    pk_plan =  (get_base_url(request).split('/plan/')[1].split('/linea'))[0]
    linea =  Linea.objects.get(pk=pk)
    if request.method == "POST":
        form = AgregarEvaluacion(request.POST)
        form.fields['actividad'].queryset = Actividad.objects.filter(linea = linea)
        if form.is_valid():
            evaluacion = form.save(commit=False)
            evaluacion.plan = PlanEstudio.objects.get(pk = pk_plan)
            evaluacion.save()
            return redirect(url_old)
    else:
        form = AgregarEvaluacion()
        form.fields['actividad'].queryset = Actividad.objects.filter(linea = linea)
    return render(request, 'alumnos/editar_evaluacion.html', {'form': form})

@login_required(login_url='/')
def editar_evaluacion(request, pk):
    url_old = get_base_url(request).split('/linea')[0]
    linea_pk =  (get_base_url(request).split('/linea/')[1].split('/evaluacion'))[0]
    linea = Linea.objects.get(pk = linea_pk)
    evaluacion = Evaluacion.objects.get(pk = pk)
    if request.method == "POST":
        form = AgregarEvaluacion(request.POST, instance=evaluacion)
        if form.is_valid():
            evaluacion = form.save(commit=False)
            evaluacion.save()
            return redirect(url_old)
